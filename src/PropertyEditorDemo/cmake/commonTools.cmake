...
function(CREATE_TRANSLATION_INT)
    cmake_parse_arguments(PARSED_ARGS "" "PROJECT_NAME" "SOURCE_DIRS;TS_FILES" ${ARGN})

    if((NOT Qt5_LUPDATE_EXECUTABLE) OR (NOT Qt5_LRELEASE_EXECUTABLE))
        message(WARNING "Package [LinguistTools] not found. Translation commands skipped")
        return()
    endif()

    if(NOT PARSED_ARGS_SOURCE_DIRS)
        message(FATAL_ERROR "You must provide SOURCE_DIRS")
    endif()

    if(NOT PARSED_ARGS_TS_FILES)
        message(FATAL_ERROR "You must provide TS_FILES")
    endif()

    if(NOT PARSED_ARGS_PROJECT_NAME)
        message(FATAL_ERROR "You must provide PROJECT_NAME")
    endif()

    add_custom_command(OUTPUT "${PROJECT_NAME}_extractTR_int"
        COMMAND ${Qt5_LUPDATE_EXECUTABLE}
        ARGS ${PARSED_ARGS_SOURCE_DIRS} -recursive -ts ${PARSED_ARGS_TS_FILES}
    )

    add_custom_command(OUTPUT "${PROJECT_NAME}_compileTR_int"
        COMMAND ${Qt5_LRELEASE_EXECUTABLE}
        ARGS ${PARSED_ARGS_TS_FILES}
    )

    add_custom_target(${PROJECT_NAME}_extractTR DEPENDS "${PROJECT_NAME}_extractTR_int")
    add_custom_target(${PROJECT_NAME}_compileTR DEPENDS "${PROJECT_NAME}_compileTR_int")
endfunction()

function(CREATE_TRANSLATION)
    cmake_parse_arguments(PARSED_ARGS "" "TS_DIR" "LANGS" ${ARGN})

    if(NOT PARSED_ARGS_LANGS)
        set(PARSED_ARGS_LANGS en ru uk es)
    endif()

    if(NOT PARSED_ARGS_TS_DIR)
        set(PARSED_ARGS_TS_DIR "${CMAKE_CURRENT_SOURCE_DIR}/translate")
    endif()

    message(${PARSED_ARGS_LANGS})

    foreach(_lang ${PARSED_ARGS_LANGS})
        list(APPEND TS_FILES "${PARSED_ARGS_TS_DIR}/${PROJECT_NAME}_${_lang}.ts")
    endforeach()

    CREATE_TRANSLATION_INT(SOURCE_DIRS ${CMAKE_CURRENT_SOURCE_DIR} TS_FILES ${TS_FILES} PROJECT_NAME ${PROJECT_NAME})
endfunction()
    ...
    foreach(TARGET ${PARSED_ARGS_TARGETS})
        ...
        if(WIN32)
            ...
            if(PROJECT_TYPE STREQUAL "EXECUTABLE")
                set(PROJECT_PREFIX "TS")
            else()
                set(PROJECT_PREFIX "Avia_")
...
function(deployTargets)
    ...
    elseif(LINUX)
        ...
        install(CODE [[
            ...
            message(STATUS "MAIN_TARGET: $ENV{DESTDIR}${MAIN_TARGET}")
            ...
            execute_process(COMMAND "${DEPLOY_TOOL_PATH}" "$ENV{DESTDIR}${MAIN_TARGET}" ${EXECUTABLE_LIST} -unsupported-allow-new-glibc
